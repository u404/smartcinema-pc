  ...
  methods: {
    ...
    getData () {
      ...
      services.getMovieData(this.id, this.act, ticketChannel).then(res => {
        ...
        if (SITE_CONFIG.movie.ticketAlert && res.isFirstGiveTicket && res.extend && res.extend.ticketAlert) {
          this.autoplay = false
          this.ticketAlert = true
        }
        if (res.extend && res.extend.forceLogin && !this.userData.uToken) {
          utils.countdown({
            total: res.extend.forceLoginDelay * 1000,
            ended: () => {
              if (res.extend.forceLoginForce) {
        ...
        if (String(err.code) === "113001001") {
          this.$router.replace({ name: "exception", params: { type: "movie_unavailable_in_region" } })
          return
        }
